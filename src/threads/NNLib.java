...
public class NNLib {
    ...
    public static Neuron[] CreateinputNeurons(int num){
        ...
        for (int i = 0; i < inputNeurons.length; i++) {
            inputNeurons[i] = new Neuron(null); //普通神经元，没有特殊胞体
        }
        return inputNeurons;
    }
    ...
    public static SpikeRecorder GenSpikeRecorder(){
        //Gen SpikeRecorder
        MinHeapQueue<Neuron> neuronQueue = new MinHeapQueue<>(); //neuron manager queue,主要就是记录发放脉冲的那些神经元
        SpikeRecorder spikeRecorder = new SpikeRecorder(neuronQueue);
        return spikeRecorder;
    }

    //使用 详细脉冲记录器和输入神经元队列 生成SNN机器装置
